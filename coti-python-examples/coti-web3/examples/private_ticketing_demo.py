...
This script     def deploy_contract(self):
        """Deploy the PrivateTicketing contract."""
        print("\n🚀 Deploying PrivateTicketing contract...")
        
        # Load contract ABI and bytecode
        contract_path = '../../../solidity/out/PrivateTicketing.sol/PrivateTicketing.json'trates how to deploy and interact with the PrivateTicketing.sol contract
...
sys.path.append(str(Path(__file__).parent.joinpath('coti-python-examples', 'coti-web3')))
...
class PrivateTicketingDemo:
    ...
    def create_event(self, name, price, total_supply, resale_allowed=False, resale_markup=0):
        ...
        # to specific users to purchase tickets. The address(0) permission doesn't
        # seem to work as expected, so we'll grant permission to ourselves.
    ...
    def purchase_ticket(self, event_id):
        ...
        try:
            print("🧪 Testing purchase with call() first...")
            result = self.contract.functions.purchaseTicket(event_id).call({'from': self.eoa.address})
        ...
        try:
            gas_estimate = self.contract.functions.purchaseTicket(event_id).estimate_gas({'from': self.eoa.address})
        ...
        func = self.contract.functions.purchaseTicket(event_id)
